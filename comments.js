...
const COMMENTS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxWJOzPrwhxtYtTRa1i3L6w3L7V76ovCXmFgmX29CX7uZrFLZ27pUWFOL4-3Mkwpmlf/exec";
  ...
  if (commentBtn && commentArea) {
    commentBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const response = await fetch(COMMENTS_SCRIPT_URL, {
          method: "POST",
          body: JSON.stringify(payload)
        });
        const resJson = await response.json().catch(() => ({ success: false, message: "Invalid response" }));

        if (!resJson.success) {
          // still continue with ad-opening flow (but inform user)
          console.warn("Comment submit returned failure:", resJson);
          alert(resJson.message || "Failed to submit comment to server. We'll still open sponsor window.");
        }
      } catch (err) {
