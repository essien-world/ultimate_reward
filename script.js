...
const OTP_SEND_URL = "https://example.com/send-otp";     // POST { phone } -> { success: true }
const OTP_VERIFY_URL = "https://example.com/verify-otp"; // POST { phone, otp } -> { success:true, matched:true }
...
async function apiCall(payload) {
  ...
  // If server returned HTML (common when URL is wrong), capture it and throw a clear error
  const text = await res.text();
  // Helpful debug: show first 300 chars of HTML so you can see what page was returned
  const snippet = text.slice(0, 300).replace(/\s+/g, " ");
  throw new Error("Expected JSON but received HTML. Check SCRIPT_URL and deployment. Response snippet: " + snippet);
}
