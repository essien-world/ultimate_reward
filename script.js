...
function setCurrentUser(user) { 
  ...
  // Do NOT store passwords or sensitive tokens. 
  try { 
    if (user) { 
      const sanitized = Object.assign({}, user); 
      // remove server-side password hash if present 
      if (sanitized.passwordHash) delete sanitized.passwordHash; 
      // Also remove any other sensitive props if present 
  ...
  try { 
    ...
    if (stored) { 
      ...
      if (parsed && parsed.phone) { 
        // restore to memory and global window 
        setCurrentUser(parsed); 
        // reflect the logged-in UI immediately, then refresh server-side data 
        updateLoginUI(); 
        // refreshUserData is async but safe to call now 
        refreshUserData().catch((e) => console.warn("refreshUserData error on init:", e)); 
      } 
    } 
  } catch (err) { 
    console.warn("Error restoring persisted user:", err); 
  }
